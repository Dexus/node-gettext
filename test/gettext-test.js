...
describe('Gettext', function() {
  ...
  describe('#loadTextdomainDirectory', function() {
    ...
    it('should load fallback', function() {
      var gt = new Gettext();
      var fileDirectory = __dirname + '/fixtures/locale_test6/';
      gt.setOptions({
        domain: 'message',
        locale: 'de_DE',
        localeDir: fileDirectory,
        localeFallback: 'en_US',
        localeDirTyp: 'GETTEXT'
      });
      gt.loadTextdomainDirectory();
      //expect(gt.getlocale()).to.equal('en_US');
      ...
      expect(gt.gettext('TESTFALLBACK')).to.equal('FALLBACK');
      expect(gt.ngettext('0 matches2', 'multiple matches2', 100)).to.equal('multiple matches2fb');
      expect(gt.ngettext('0 matches2', 'multiple matches2', 1)).to.equal('0 matches2fb');
      expect(gt.getlocale()).to.equal('de_DE');
    });
