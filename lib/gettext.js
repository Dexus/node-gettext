...
function Gettext(options) {
  ...
  this._options.localeFallback = options && options.localeFallback || false;
  ...
  this._pluralsFallbackInfo = false;
  this._pluralsFallbackFunc = false;


  if (this._options.localeFallback) {

    var defaultLocale = this._locale;

    this._locale = this._options.localeFallback;

    var locale = extend([], this._locale.split(/[-_\.@]/));


    var mo = creatDirStyle(this._options.localeDir, locale.join('_'), this._currentDomain, this._options.localeDirTyp);

    while (!fs.existsSync(mo) && locale.length) {
      locale.pop();
      mo = creatDirStyle(this._options.localeDir, locale.join('_'), this._currentDomain, this._options.localeDirTyp);
    }
    var locale_short = normalizeLocale(locale.join('_'), true);
    if (locale.length && plurals[locale_short]) {
      this._pluralsFallbackInfo = plurals[locale_short];
      this._pluralsFallbackFunc = plurals[locale_short].pluralsFunc;
      this._options.localeFallback = locale.join('_');
      this.loadTextdomain(this._currentDomain, mo);
    }
...
Gettext.prototype.setOptions = function(options) {
  ...
  this._options.localeFallback = options && options.localeFallback || this._options.localeFallback;
...
Gettext.prototype.dnpgettext = function(domain, msgctxt, msgid, msgidPlural, count) {
  ...
  if (translation) {
    ...
    if (this._options.localeFallback && this._pluralsFallbackInfo) {
      var reUseLocale = this._locale;
      ...
      this._locale = reUseLocale;
      this.loadTextdomainDirectory(domain);

      if (translation) {
        if (typeof count === 'number') {
          index = this._pluralsFallbackFunc(count);
          if (typeof index === 'boolean') {
            index = index ? 1 : 0;
          }
        } else {
